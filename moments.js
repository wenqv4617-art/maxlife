...
const groups = [...new Set(chars.map(c => c.group || "默认"))];
  ...
  if (window.currentConversationId === convId && window.loadConversationMessages) {
    await window.loadConversationMessages(convId);
  }
});
  }
  ...
  async function ensureMomentsPageElements() {
    ...
    page.innerHTML = `
      ...
      <div class="moments-toolbar">
        <button class="mm-btn primary" id="momentsNewPostBtn"><span style="display:inline-flex;align-items:center;gap:6px;">${Icons.edit}<span>发布动态</span></span></button>
      </div>
