...
async function saveForumData(data) {
    // 直接序列化存，不走可能冲突的 DB.put
    const d = await window.DB.openDB();
    return new Promise((resolve, reject) => {
        const tx = d.transaction('forum', 'readwrite');
        tx.objectStore('forum').put({ key: 'main', value: data });
...
async function submitComment(postId, text, replyToHandle = '') {
    ...
    await saveForumData(saveData);
    window._forumDataCache = saveData;
        ...
        try {
            ...
            if (replies && replies.length > 0) {
                ...
                replies.forEach(r => {
                    ...
                    if (replyTo.replace('@', '') === accountHandle) {
                ...
                // 🔧 深拷贝后保存
                const saveData2 = JSON.parse(JSON.stringify(d));
                await saveForumData(saveData2);
                window._forumDataCache = saveData2;
