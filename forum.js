...
async function renderMsgList(friendsOnly = false) {
    ...
    const displayMsgs = msgs.filter(m => {
        if (m.isPlaceholder && (!m.content || m.content.trim() === '') && (!m.bubbles || m.bubbles.length === 0 || !m.bubbles[0].content)) {
            // 检查是否有其他真实消息来自同一人
            const hasRealMsg = msgs.some(other => 
                other !== m && other.from === m.from && 
                (!other.isPlaceholder || (other.content && other.content.trim()) || (other.bubbles && other.bubbles.length > 0 && other.bubbles[0].content))
            );
            return hasRealMsg ? false : true; // 如果没有真实消息，保留占位
        }
        return true;
    });
    ...
    filteredMsgs.forEach(m => {
        ...
        if (!conversations[key]) {
            conversations[key] = { from: key, avatar: m.avatar || '', lastMsg: m, allMsgs: [] };
        }
        conversations[key].allMsgs.push(m);
        // 用真实时间戳判断最新消息
        const getTs = (msg) => msg?.timestamp || 0;
        if (!conversations[key].lastMsg || getTs(m) > getTs(conversations[key].lastMsg)) {
    ...
    listEl.innerHTML = convArray.map(c => {
        ...
        const fromHandle = c.lastMsg?.fromHandle || '';
        ...
        <div class="msg-list-item" data-from="${window.escapeHtml(c.from)}">
            ...
            <div class="msg-list-info">
                ...
                <div class="msg-list-preview">${window.escapeHtml((c.lastMsg.content || '').substring(0, 50))}${(c.lastMsg.content || '').length > 50 ? '...' : ''}</div>
            </div>
        </div>`;
    }).join('');
...
async function sendMessageLocal() {
    ...
    data.messages = data.messages.filter(m => {
        if (m.isPlaceholder && m.from === fromName && m.to === (account?.name || '')) return false;
        if (m.isPlaceholder && m.from === (account?.name || '') && m.to === fromName) return false;
        return true;
    });
...
function buildMessageData(fromName) {
    ...
    if (msgType === 'voice') {
        return { id: 'msg_' + ts, from: account?.name || '我', to: fromName, content: '[语音] ' + text, time: '刚刚', timestamp: ts, avatar: account?.avatar || '', bubbles: [{ type: 'voice', content: text, transcript: text, duration: '0:05' }] };
    } else if (msgType === 'image') {
        return { id: 'msg_' + ts, from: account?.name || '我', to: fromName, content: '[图片] ' + text, time: '刚刚', timestamp: ts, avatar: account?.avatar || '', bubbles: [{ type: 'image', content: text, caption: text }] };
    }
    return { id: 'msg_' + ts, from: account?.name || '我', to: fromName, content: text, time: '刚刚', timestamp: ts, avatar: account?.avatar || '', bubbles: [{ type: 'text', content: text }] };
}
...
function openAccountEdit(accountId = null) {
    ...
    let account;
    if (accountId) account = data.accounts.find(a => a.id === accountId);
...
async function addNotification(content, postId = null) {
    ...
    data.notifications.unshift({ id: 'notif_' + Date.now(), time: '刚刚', content, postId });
...
async function submitComment(postId, text, replyToHandle = '') {
    ...
    setTimeout(async () => {
        ...
        try {
            if (window.recordApiPending) window.recordApiPending();
            const prompt = `${context}\n\n有人评论了帖子："${text}"，评论者是@${account.handle}\n\n生成1-2条回复。JSON：[{"name":"","handle":"","text":"","replyTo":""}]\n禁止使用name="${account?.name || ''}"、handle="${account?.handle || ''}"`;
            console.log('📩 submitComment AI请求 prompt:', prompt.substring(0, 200));
            const reply = await window.callLLM([{ role: 'user', content: prompt }], { temperature: 1.0, maxTokens: 400 });
            console.log('📩 submitComment AI原始回复:', reply);
            const replies = parseJSONFromReply(reply);
            console.log('📩 submitComment 解析后:', JSON.stringify(replies));
            if (replies && replies.length > 0) {
                const d = await getForumData();
                if (!d.comments) d.comments = {};
                if (!d.comments[postId]) d.comments[postId] = [];
                console.log('📩 AI返回的原始replies:', JSON.stringify(replies));
            console.log('📩 当前评论列表(替换前):', JSON.stringify(d.comments[postId].map(c => ({
                handle: c.handle, 
            ...
            replies.forEach(r => {
                ...
                const newReply = { 
                    ...
                    avatar: '', 
                    text: replyTo ? `回复 @${replyTo.replace('@', '')}：${text}` : text, 
                    replyTo 
                };
                
                console.log('📩 处理回复:', JSON.stringify({replyTo, targetHandle: replyTo.replace('@', ''), text: text.substring(0,20), accountHandle: account?.handle}));
                
                if (replyTo) {
                    const targetHandle = replyTo.replace('@', '');
                    let parentComment = null;
                    
                    // 🔧 优先匹配你自己（刚发的评论）
                    const accountHandle = account?.handle || '';
                    if (targetHandle === accountHandle) {
                        // 在你的评论中找最新的一条
                        const userComments = d.comments[postId].filter(c => c.handle === accountHandle);
                        console.log('📩 匹配你的评论, 找到', userComments.length, '条');
                        if (userComments.length > 0) {
                            parentComment = userComments[userComments.length - 1];
                            console.log('📩 选中的父评论:', parentComment.handle, (parentComment.text||'').substring(0,20));
                        }
                    }
                    
                    // 如果没匹配到你的，再搜索所有评论
                    if (!parentComment) {
                        console.log('📩 未匹配到你的评论，搜索所有评论...');
                        for (const comment of d.comments[postId]) {
                            if (comment.handle === targetHandle) {
                                parentComment = comment;
                                console.log('📩 在顶层找到:', comment.handle);
                                break;
                            }
                            if (comment.replies) {
                                for (const subReply of comment.replies) {
                                    if (subReply.handle === targetHandle) {
                                        parentComment = comment;
                                        console.log('📩 在回复中找到:', subReply.handle, '父:', comment.handle);
                                        break;
                                    }
                                }
                    ...
                    if (parentComment) {
                        ...
                        console.log('✅ 回复已挂载到:', parentComment.handle, '目前有', parentComment.replies.length, '条回复');
                    } else {
                        console.log('⚠️ 找不到父评论，默认回复你的评论');
                        // 🔧 找不到目标，默认作为回复你的评论
                        if (accountHandle) {
                            const userComments = d.comments[postId].filter(c => c.handle === accountHandle);
                            if (userComments.length > 0) {
                                ...
                                newReply.text = `回复 @${accountHandle}：${text}`;
                                newReply.replyTo = accountHandle;
                                target.replies.push(newReply);
                                console.log('✅ 默认挂载到你的评论:', target.handle);
                            } else {
                                d.comments[postId].push(newReply);
                                console.log('✅ 作为顶层评论添加');
                            }
                        } else {
                            d.comments[postId].push(newReply);
                            console.log('✅ 作为顶层评论添加(无账户)');
                        }
                    }
                } else {
                    d.comments[postId].push(newReply);
                    console.log('✅ 无replyTo，作为顶层评论');
                }
                
                if (replyTo.replace('@', '') === (account?.handle || '')) {
                    addNotification(`${r.name || '匿名'} 回复了你的评论`, postId);
                }
            });
            
            console.log('📩 评论列表(替换后):', JSON.stringify(d.comments[postId].map(c => ({
                handle: c.handle, 
                text: (c.text||'').substring(0,30), 
                replies: (c.replies||[]).length
            }))));
                const p = d.posts.find(p => p.id === postId);
                if (p) p.comments = (d.comments[postId] || []).reduce((sum, c) => sum + 1 + (c.replies ? c.replies.length : 0), 0);
                console.log('📩 保存后评论总数:', (d.comments[postId] || []).length);
            console.log('📩 保存后评论详情:', JSON.stringify((d.comments[postId] || []).map(c => ({handle: c.handle, text: (c.text||'').substring(0,30), replies: (c.replies||[]).length}))));
            
            // 🔧 序列化检查
            const jsonStr = JSON.stringify(d);
            console.log('📩 序列化后replies数量:', (d.comments[postId] || []).reduce((sum, c) => sum + (c.replies?.length || 0), 0));
            const parsedBack = JSON.parse(jsonStr);
            console.log('📩 反序列化后replies数量:', (parsedBack.comments?.[postId] || []).reduce((sum, c) => sum + (c.replies?.length || 0), 0));
            
            await saveForumData(d);
            window._forumDataCache = d;
            
            // 🔧 验证保存是否成功
            const verifyData = await getForumData();
                console.log('📩 验证保存后的评论:', postId, JSON.stringify((verifyData.comments || {})[postId]?.map(c => ({
                    handle: c.handle,
                    text: (c.text||'').substring(0,30),
                    replies: (c.replies||[]).length
                }))));
                ...
                await renderPostList();
                if (window._currentDetailPostId === postId) {
                    console.log('📩 刷新详情页, postId:', postId);
                    // 🔧 确保使用最新数据
                    window._forumDataCache = verifyData;
                    await openPostDetailDirect(postId);
                } else {
                    console.log('📩 未刷新详情页, currentDetailPostId:', window._currentDetailPostId);
                }
            }
        } catch (e) { /* 静默 */ }
        ...
        // ========== 自动触发私信生成 ==========
        setTimeout(async () => {
            await generateAutoMessages('comment', { postId, text, replyToHandle });
        }, 2000);
    }, 2000);
}
...
function bindForumEvents() {
    ...
    document.getElementById('profileEditBtn').addEventListener('click', () => openAccountEdit());
