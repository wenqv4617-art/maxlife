...
async function submitComment(postId, text, replyToHandle = '') {
    ...
    if (replyToHandle) {
        ...
        for (const c of data.comments[postId]) {
            if (c.handle === replyToHandle) {
                targetComment = c;
                break;
            }
            if (c.replies) {
                for (const r of c.replies) {
                    if (r.handle === replyToHandle) {
                        targetComment = c;
                        break;
                    }
                }
    ...
    const post = data.posts.find(p => p.id === postId);
    if (post) post.comments = (data.comments[postId] || []).reduce((sum, c) => sum + 1 + (c.replies ? c.replies.length : 0), 0);
    ...
    setTimeout(async () => {
        const context = await buildForumContext(false);
        try {
            const prompt = `${context}\n\n有人评论了帖子："${text}"，评论者是@${account.handle}\n\n生成1-2条回复。JSON：[{"name":"","handle":"","text":"","replyTo":"@${account.handle}"}]\n禁止使用name="${account?.name || ''}"、handle="${account?.handle || ''}"`;
            ...
            if (replies && replies.length > 0) {
                ...
                replies.forEach(r => {
                    const replyTo = r.replyTo || `@${account.handle}`;
                    ...
                    if (targetHandle === (account?.handle || '')) {
                ...
                const p = d.posts.find(p => p.id === postId);
                if (p) {
                    p.comments = (d.comments[postId] || []).reduce((sum, c) => sum + 1 + (c.replies ? c.replies.length : 0), 0);
                }
