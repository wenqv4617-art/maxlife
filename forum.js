...
async function renderMsgList() {
    ...
    msgs.forEach(m => {
        ...
        if (!conversations[key].lastMsg || (m.time && conversations[key].lastMsg.time && m.time > conversations[key].lastMsg.time)) {
...
// ==================== 私信对话 ====================
async function openConversation(fromName) {
    const notifCard = document.querySelector('.msg-notification-card');
    if (notifCard) notifCard.remove();
...
function showBubbleDetail(title, content) {
    ...
    overlay.innerHTML = `
        <div class="msg-detail-card">
            <div class="detail-type-icon">${title.startsWith('🎤') ? '🎤' : '🖼️'}</div>
            <div style="font-weight:600;font-size:17px;margin-bottom:10px;">${title}</div>
            <div class="detail-text">${window.escapeHtml(content || '暂无详情')}</div>
            <button class="detail-close-btn">关闭</button>
        </div>`;
    (document.getElementById('page-forum') || document.body).appendChild(overlay);
    overlay.addEventListener('click', (e) => {
        if (e.target === overlay || e.target.classList.contains('detail-close-btn')) overlay.remove();
    });
}
        ...
        try {
            ...
            if (bubbles && Array.isArray(bubbles) && bubbles.length > 0) {
                ...
                if (window._currentConversationFrom === fromName && document.getElementById('forumMsgConversationView').style.display === 'flex') {
                    await openConversation(fromName);
                }
            }
        } catch (e) { /* 静默 */ }
    }, 1500);
...
function buildMessageData(fromName) {
    ...
    if (msgType === 'voice') {
        return { id: 'msg_' + Date.now(), from: account?.name || '我', to: fromName, content: '[语音] ' + text, time: '刚刚', avatar: account?.avatar || '', bubbles: [{ type: 'voice', content: text, transcript: text, duration: '0:05' }] };
    } else if (msgType === 'image') {
        return { id: 'msg_' + Date.now(), from: account?.name || '我', to: fromName, content: '[图片] ' + text, time: '刚刚', avatar: account?.avatar || '', bubbles: [{ type: 'image', content: text, caption: text }] };
    }
    return { id: 'msg_' + Date.now(), from: account?.name || '我', to: fromName, content: text, time: '刚刚', avatar: account?.avatar || '', bubbles: [{ type: 'text', content: text }] };
}
    ...
    detailEl.innerHTML = `
        <div class="detail-tweet">
            ...
            <div class="detail-stats-bar">
                ...
                <span><span class="detail-stat-num">${post.likes || 0}</span> 喜欢</span>
    ...
    setTimeout(() => {
        ...
        if (avatarEl) {
            avatarEl.addEventListener('click', async (e) => {
                ...
                if (authorHandle || authorName) await generateAuthorProfile(authorHandle, authorName);
            });
        }
    }, 100);
...
// ==================== 通知 ====================
async function addNotification(content) {
    const data = await getForumData();
    if (!data.notifications) data.notifications = [];
    data.notifications.unshift({ id: 'notif_' + Date.now(), time: '刚刚', content });
...
async function generatePostInteraction(postId) {
    ...
    try {
        ...
        if (newComments && newComments.length > 0) {
            ...
            newComments.forEach(c => {
                const replyTo = c.replyTo || '';
                if (replyTo) addNotification(`${c.name || '匿名'} 回复了 @${replyTo.replace('@', '')}`);
                else addNotification(`${c.name || '匿名'} 评论了帖子`);
            });
            addNotification(`帖子获得了 ${p?.likes || 0} 个喜欢`);
        }
...
async function generateAuthorProfile(authorHandle, authorName) {
    ...
    try {
        ...
        if (result && result.posts) {
            ...
            openAuthorProfile(authorHandle, authorName, result.profile?.bio || '', existingPosts.length + newPosts.length);
        }
...
async function submitPostAsAuthor(authorHandle, authorName) {
    ...
    try {
        ...
        if (postData && postData.content) {
            ...
            openAuthorProfile(authorHandle, authorName, '', currentPosts.length);
            addNotification(`${authorName} 发布了一条新帖子`);
    ...
    setTimeout(() => {
        ...
        if (postBtn) {
            postBtn.addEventListener('click', async (e) => {
                e.stopPropagation();
                await submitPostAsAuthor(postBtn.dataset.authorHandle, postBtn.dataset.authorName);
            });
        }

        document.querySelectorAll('#forumDetailContent .tweet-card').forEach(card => {
            card.addEventListener('click', (e) => {
                if (e.target.closest('.tweet-stat') || e.target.closest('#followAuthorBtn')) return;
                if (card.dataset.postId) openPostDetail(card.dataset.postId);
            });
        });
    }, 200);
}
...
async function generateMessages() {
    ...
    try {
        if (window.recordApiPending) window.recordApiPending();
        const reply = await window.callLLM([{ role: 'user', content: `${context}\n\n生成2-4条发给@${account?.handle || 'user'}的私信。JSON：[{"from":"","fromHandle":"","fromBio":"","time":"","bubbles":[{"type":"text","content":"消息内容"}]}]` }], { temperature: 1.0, maxTokens: 500 });
        const msgs = parseJSONFromReply(reply);
        if (msgs && msgs.length > 0) {
            const data = await getForumData();
            const newMsgs = msgs.map(m => ({ ...m, id: 'msg_' + Date.now() + '_' + Math.random().toString(36).slice(2, 6), fromHandle: m.fromHandle || '', fromBio: m.fromBio || '' }));
        ...
        try {
            ...
            if (comments && comments.length > 0) {
                ...
                if (post?.handle === account?.handle) {
                    comments.forEach(c => addNotification(`${c.name || '匿名'} 评论了你的帖子`));
                    addNotification(`你的帖子获得了 ${post?.likes || 0} 个喜欢`);
                }
            }
        } catch (e) { /* 静默 */ }
    }, 2000);
...
async function submitComment(postId, text) {
    ...
    const comment = { id: 'cmt_' + Date.now(), name: account.name, handle: account.handle, avatar: account.avatar || '', text: text.trim() };
    ...
    addNotification(`你评论了 @${post?.handle || 'unknown'} 的帖子`);
    ...
    setTimeout(async () => {
        ...
        try {
            if (window.recordApiPending) window.recordApiPending();
            if (window.recordApiPending) window.recordApiPending();
            const reply = await window.callLLM([{ role: 'user', content: `${context}\n\n有人评论了帖子："${text}"，评论者是@${account.handle}\n\n生成1-2条回复。JSON：[{"name":"","handle":"","text":"","replyTo":""}]\n禁止使用name="${account?.name || ''}"、handle="${account?.handle || ''}"` }], { temperature: 1.0, maxTokens: 400 });
            ...
            if (replies && replies.length > 0) {
                ...
                replies.forEach(r => {
                    ...
                    const newReply = { id: 'cmt_' + Date.now(), name: r.name || '匿名', handle: r.handle || 'user', avatar: '', text: replyTo ? `回复 @${replyTo.replace('@', '')}：${r.text || ''}` : (r.text || ''), replyTo };
                    d.comments[postId].push(newReply);
                    if (replyTo.replace('@', '') === (account?.handle || '')) {
                        addNotification(`${r.name || '匿名'} 回复了你的评论`);
                    }
                });
                const p = d.posts.find(p => p.id === postId);
                if (p) p.comments = (d.comments[postId] || []).reduce((sum, c) => sum + 1 + (c.replies ? c.replies.length : 0), 0);
                await saveForumData(d);
                window._forumDataCache = d;
                if (window._currentDetailPostId === postId) await openPostDetail(postId);
            }
        } catch (e) { /* 静默 */ }
    }, 2000);
}
...
function bindForumEvents() {
    ...
    document.querySelectorAll('.forum-bottom-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (view === 'main') { switchForumView('forumMainView');
                ...
                renderNotifList();
                document.getElementById('notifBadge').style.display = 'none'; } else if (view === 'msg') { switchForumView('forumMsgView');
                renderMsgList(); }
        });
    });
    ...
    // 详情返回
    document.getElementById('forumDetailBackBtn').addEventListener('click', () => {
    ...
    document.getElementById('forumMsgConvBackBtn').addEventListener('click', () => {
        document.getElementById('forumMsgConversationView').style.display = 'none';
        document.getElementById('forumMsgView').style.display = 'block';
        document.getElementById('forumBottomNav').style.display = 'flex';
        document.getElementById('forumFabBtn').style.display = 'block';
    ...
    // 个人主页
    document.getElementById('forumProfileBackBtn').addEventListener('click', () => { switchForumView('forumMainView');
        updateBottomNavActive('main'); });
