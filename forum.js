...
async function openPostDetailDirect(postId) {
    ...
    detailEl.innerHTML = `
        ...
        <div style="padding:12px 16px;font-weight:600;font-size:15px;border-bottom:1px solid #eee;">💬 评论 (${comments.length})</div>
        <div class="comments-list-wrapper">
            ${comments.map((c, ci) => `
                <div class="comment-group" style="border-bottom: 1px solid #eee;">
                    <div class="comment-item clickable-comment" data-comment-index="${ci}" data-comment-handle="${window.escapeHtml(c.handle || '')}" data-comment-name="${window.escapeHtml(c.name || '')}" style="cursor:pointer; padding:12px 16px;">
                    ...
                    ${(c.replies && c.replies.length > 0) ? `
                        <div class="replies-section" style="margin-left: 48px; margin-bottom: 12px; background: #f8f9fa; border-left: 2px solid #1d9bf0; border-radius: 0 0 8px 8px;">
                            ${c.replies.map(r => `
                                <div class="comment-item" style="padding: 8px 12px; border-bottom: 1px solid #f0f0f0;">
                                    <div class="comment-avatar" style="width:28px;height:28px;background-image:url('${r.avatar || ''}');"></div>
                                    <div class="comment-body">
                                        <span class="comment-name" style="font-size:13px;">${window.escapeHtml(r.name || '匿名')}</span>
                                        <span class="comment-handle" style="font-size:12px;">@${window.escapeHtml(r.handle || '')}</span>
                                        <div class="comment-text" style="font-size:14px; color:#333;">${window.escapeHtml(r.text || '')}</div>
                                    </div>
                                </div>
                            `).join('')}
                        </div>
