...
async function renderProfilePosts(account) {
    ...
    listEl.querySelectorAll('.tweet-card').forEach(card => {
        card.addEventListener('click', (e) => {
            if (e.target.closest('.tweet-stat')) return;
            // 设置来源为个人主页，以便返回时回到主页
            window._detailFromAuthorProfile = false;
            window._forumNavStack = [];
            openPostDetail(card.dataset.postId);
        });
    });
...
async function openAuthorProfile(authorHandle, authorName, authorBio, postCount) {
    if (document.getElementById('forumDetailView').style.display !== 'none' && window._currentAuthorProfile) {
        window._forumNavStack.push({ type: 'profile', ...window._currentAuthorProfile });
    }
    window._detailFromAuthorProfile = false;
    ...
    window._currentAuthorProfile = { handle: authorHandle, name: authorName, bio: authorBio, postCount };
    switchForumView('forumDetailView');
    const commentInputRow = document.getElementById('forumDetailCommentInput');
        ...
        document.querySelectorAll('#forumDetailContent .tweet-card').forEach(card => {
            card.addEventListener('click', (e) => {
                ...
                if (card.dataset.postId) {
                    window._detailFromAuthorProfile = true;
                    openPostDetail(card.dataset.postId);
                }
            });
        });
...
function bindForumEvents() {
    ...
    document.getElementById('forumDetailBackBtn').addEventListener('click', async () => {
        ...
        // 检查导航栈（从他人主页进入）
        if (window._forumNavStack && window._forumNavStack.length > 0) {
            const prev = window._forumNavStack.pop();
            window._detailFromAuthorProfile = false;
            if (prev.type === 'profile') {
                openAuthorProfile(prev.handle, prev.name, prev.bio, prev.postCount);
                return;
            }
        }
        
        // 检查是否从他人主页进入
        if (window._detailFromAuthorProfile && window._currentAuthorProfile) {
            window._detailFromAuthorProfile = false;
            const profile = window._currentAuthorProfile;
            openAuthorProfile(profile.handle, profile.name, profile.bio, profile.postCount);
            return;
        }
        
        // 检查是否在自己的个人主页（从我的帖子/我的喜欢进入）
        if (document.getElementById('forumProfileView').style.display !== 'none' || 
            (window._profileTab && (document.getElementById('forumProfileView').style.display === 'none' && !window._forumNavStack.length && !window._detailFromAuthorProfile))) {
            // 默认返回论坛首页，但如果个人主页还在栈中则回到个人主页
            switchForumView('forumMainView');
            updateBottomNavActive('main');
            return;
        }
