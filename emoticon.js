...
window.initEmoticonModule = function({ DB, showStatus, escapeHtml, getAvatarColor, compressImage, callLLM, switchPage }) {
    ...
    const id = await new Promise((resolve, reject) => {
        ...
        req.onsuccess = e => {
            ...
            const addReq = tx.objectStore('emoticonItems').add(item);
            addReq.onsuccess = () => {
                db.close();
    ...
    async function buildEmoticonSection(convId) {
        ...
        let section = '\n\n【可用表情包】\n你可以使用以下表情包来表达情绪。在消息中使用格式：[EMOTICON:文字说明]\n';
    ...
    function getEmoticonPromptRule(hasEmoticon) {
        if (hasEmoticon) {
            return '你可以偶尔使用表情包，格式为 [EMOTICON:文字说明]，且文字说明必须完全来自【可用表情包】列表中列出的内容，不要自创。';
        }
        return '不要使用表情包格式。';
    }
