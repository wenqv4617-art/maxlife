...
(function initGroupChat() {
    ...
    async function ensureGroupStores() {
        const d = await window.DB._rawDB || await new Promise((resolve, reject) => {
            const req = indexedDB.open("CompanionDB_V18", 20);
            req.onsuccess = e => resolve(e.target.result);
            req.onerror = e => reject(e.target.error);
        });
        const stores = ['groupChats', 'groupMessages', 'groupNPCs', 'groupMemories'];
        const needUpgrade = stores.some(s => !d.objectStoreNames.contains(s));
        if (needUpgrade) {
            d.close();
            const newVersion = d.version + 1;
            await new Promise((resolve, reject) => {
                const req = indexedDB.open("CompanionDB_V18", newVersion);
...
- 直接以叙事文本输出。描述谁做了什么、说了什么、发生了什么。像写小说一样`;
    ...
    function bindEvents() {
        ...
        document.getElementById('setupParticipateYes')?.addEventListener('click', () => { document.getElementById('setupParticipateYes').classList.add('selected');
            document.getElementById('setupParticipateNo').classList.remove('selected'); });
        document.getElementById('setupParticipateNo')?.addEventListener('click', () => { document.getElementById('setupParticipateNo').classList.add('selected');
        ...
        document.getElementById('groupExpandMenuBtn')?.addEventListener('click', () => {
            ...
            const g = window.DB.get('groupChats', window.currentGroupId).then(g => {
                ...
                if (g.isUserParticipating) {
                    menu.innerHTML = `<div class="expand-menu-item" data-action="groupVoice"><span class="expand-menu-icon">🎤</span><span class="expand-menu-label">语音</span></div><div class="expand-menu-item" data-action="groupImage"><span class="expand-menu-icon">🖼️</span><span class="expand-menu-label">图片</span></div><div class="expand-menu-item" data-action="groupEmoticon"><span class="expand-menu-icon">😊</span><span class="expand-menu-label">表情</span></div><div class="expand-menu-item" data-action="groupToggleMode"><span class="expand-menu-icon">🔄</span><span class="expand-menu-label">见面</span></div><div class="expand-menu-item" data-action="groupTransfer"><span class="expand-menu-icon">💸</span><span class="expand-menu-label">转账</span></div><div class="expand-menu-item" data-action="groupRedPacket"><span class="expand-menu-icon">🧧</span><span class="expand-menu-label">发红包</span></div><div class="expand-menu-item" data-action="groupSummary"><span class="expand-menu-icon">📋</span><span class="expand-menu-label">总结</span></div><div class="expand-menu-item" data-action="groupOpenDetail"><span class="expand-menu-icon">📝</span><span class="expand-menu-label">详情</span></div>`;
                } else {
                    menu.innerHTML = `<div class="expand-menu-item" data-action="groupSummary"><span class="expand-menu-icon">📋</span><span class="expand-menu-label">总结</span></div><div class="expand-menu-item" data-action="groupOpenDetail"><span class="expand-menu-icon">📝</span><span class="expand-menu-label">详情</span></div>`;
                }
    ...
    function bindExpandMenuActions() {
        document.querySelectorAll('#groupExpandMenu .expand-menu-item').forEach(el => el.addEventListener('click', () => {
            ...
            if (a === 'groupVoice') { const c = prompt('语音内容：'); if (c) { window.DB.put('groupMessages', { groupId: window.currentGroupId, senderId: 'user', senderName: '我', role: 'user', content: c, messageType: 'voice', timestamp: Date.now() });
                    loadGroupMessages(window.currentGroupId); } }
            if (a === 'groupImage') { const d = prompt('图片描述：'); if (d) { window.DB.put('groupMessages', { groupId: window.currentGroupId, senderId: 'user', senderName: '我', role: 'user', content: d, messageType: 'image', timestamp: Date.now() });
                    loadGroupMessages(window.currentGroupId); } }
        }));
    }
