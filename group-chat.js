...
function renderGroupMediaBubble(kind, content) {
    ...
    return `
        <div class="bubble voice-bubble clickable" data-voice-content="${window.escapeHtml(content || '')}">
            ...
            <div class="voice-content-hidden" style="display:none;">${window.escapeHtml(content || '')}</div>
        </div>
    `;
}
...
function bindGroupBubbleToolbar(container, msgs) {
    ...
    container.querySelectorAll('.voice-bubble[data-voice-content]').forEach(card => {
        card.addEventListener('click', (e) => {
            ...
            panel.style.cssText = 'margin-top:8px;padding:8px 10px;background:rgba(255,255,255,0.55);border-radius:10px;font-size:13px;color:#4a5568;line-height:1.5;border-left:3px solid rgba(74,85,104,0.18);word-break:break-word;';
