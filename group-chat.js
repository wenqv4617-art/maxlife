                ...
                if (m.type === 'system') { h += `<div class="group-system-msg">${window.escapeHtml(m.content)}</div>`; }
                    ...
                    h += `<div style="position:relative;" data-offline-msg-id="${m.id||''}">
                        <div class="bubble-toolbar offline-toolbar" data-toolbar-for="${i}">
                            <button class="toolbar-btn reback-btn" data-idx="${i}">重回</button>
                            <button class="toolbar-btn danger delete-msg-btn" data-idx="${i}">删除</button>
                            <button class="toolbar-btn multi-select-btn" data-idx="${i}">多选</button>
                            <button class="toolbar-btn edit-msg-btn" data-idx="${i}">编辑</button>
                        </div>
 ...
 * 4. 按当前面具隔离收藏
 * ================================================================ */
(function initFavoriteToolbarEnhance() {
    "use strict";

    const FAV_ICON = '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>';

    const TOOLBAR_LABELS = [
        { cls: 'edit-msg-btn', text: '编辑', order: 1 },
        { cls: 'view-msg-btn', text: '查看', order: 2 },
        { cls: 'reback-btn', text: '重回', order: 3 },
        { cls: 'delete-msg-btn', text: '删除', order: 4, danger: true },
        { cls: 'multi-select-btn', text: '多选', order: 5 }
    ];
    ...
    async function getFavoriteKey() {
        return 'messageFavorites_' + await getMaskId();
    }

    async function getFavorites() {
        const key = await getFavoriteKey();
        const list = await window.DB.getSetting(key, []);
        return Array.isArray(list) ? list : [];
    }

    async function setFavorites(list) {
        const key = await getFavoriteKey();
        await window.DB.setSetting(key, list);
    }

    function makeFavoriteId(sourceType, sourceId, msgId) {
        return `${sourceType}_${sourceId}_${msgId}`;
    }

    function getToolbarFromTarget(target) {
        const row =
            target.closest('.message-row') ||
            target.closest('.group-message-row') ||
            target.closest('[data-offline-msg-id]');

        if (!row) return null;
        return row.querySelector('.bubble-toolbar');
    }

    function getContextFromTarget(target) {
        const singleRow = target.closest('.message-row');
        if (singleRow) {
            const msgId = singleRow.dataset.messageId;
            if (!msgId) return null;
            return {
                sourceType: 'single',
                sourceId: window.currentConversationId || window._currentLoadingConvId,
                msgId: parseInt(msgId),
                row: singleRow
            };
        }

        const singleOffline = target.closest('#convChatMessages [data-offline-msg-id]');
        if (singleOffline) {
            const msgId = singleOffline.dataset.offlineMsgId;
            if (!msgId) return null;
            return {
                sourceType: 'single',
                sourceId: window.currentConversationId || window._currentLoadingConvId,
        ...
        const groupRow = target.closest('.group-message-row');
        if (groupRow) {
            const msgId = groupRow.dataset.msgId;
            if (!msgId) return null;
            return {
                sourceType: 'group',
                sourceId: window.currentGroupId,
                msgId: parseInt(msgId),
                row: groupRow
            };
        }

        const groupOffline = target.closest('#groupChatMessages [data-offline-msg-id]');
        if (groupOffline) {
            const msgId = groupOffline.dataset.offlineMsgId;
            if (!msgId) return null;
            return {
                sourceType: 'group',
                sourceId: window.currentGroupId,
    ...
    async function getMessageRecord(ctx) {
        if (!ctx) return null;
        if (ctx.sourceType === 'single') {
            return await window.DB.get('chats', ctx.msgId);
        }
        return await window.DB.get('groupMessages', ctx.msgId);
    }

    async function getSourceTitle(ctx) {
        if (!ctx) return '未知对话';

        if (ctx.sourceType === 'single') {
            const conv = await window.DB.get('conversations', ctx.sourceId);
            if (!conv) return '未知对话';

            const cd = await window.DB.get('convDetails', ctx.sourceId);
            const ch = await window.DB.get('characters', conv.charId);
            return cd?.charName || ch?.name || conv.name || '单聊';
        }

        const g = await window.DB.get('groupChats', ctx.sourceId);
        return g?.name || '群聊';
    }

    async function getSourceAvatar(ctx) {
        if (!ctx) return { text: '?', image: '', color: '#8ba3c7' };

        if (ctx.sourceType === 'single') {
            const conv = await window.DB.get('conversations', ctx.sourceId);
            if (!conv) return { text: '?', image: '', color: '#8ba3c7' };

            const cd = await window.DB.get('convDetails', ctx.sourceId);
            const ch = await window.DB.get('characters', conv.charId);
            const name = cd?.charName || ch?.name || '单';
            const image = cd?.charAvatar || ch?.avatar || '';
            return {
                text: name.charAt(0),
                image,
                color: window.getAvatarColor ? window.getAvatarColor(name) : '#8ba3c7'
            };
        }
    ...
    async function isFavorited(ctx) {
        if (!ctx) return false;
        const list = await getFavorites();
        const id = makeFavoriteId(ctx.sourceType, ctx.sourceId, ctx.msgId);
        return list.some(x => x.id === id);
    }

    async function toggleFavorite(ctx, btn) {
        if (!ctx || !window.DB) return;

        const msg = await getMessageRecord(ctx);
        if (!msg) {
            window.showStatus && window.showStatus('消息不存在', 'error');
            return;
        }

        const list = await getFavorites();
        const favId = makeFavoriteId(ctx.sourceType, ctx.sourceId, ctx.msgId);
        const oldIndex = list.findIndex(x => x.id === favId);
        ...
        if (oldIndex >= 0) {
            ...
            window.showStatus && window.showStatus('已取消收藏', 'success');
            return;
        }

        const title = await getSourceTitle(ctx);
        ...
        const item = {
            ...
            sourceTitle: title,
            msgId: ctx.msgId,
            store: ctx.sourceType === 'single' ? 'chats' : 'groupMessages',
            content: msg.content || '',
            messageType: msg.messageType || msg.type || 'text',
            role: msg.role || '',
            senderId: msg.senderId || '',
            senderName: msg.senderName || (msg.role === 'user' ? '我' : '对方'),
        ...
        window.showStatus && window.showStatus('已收藏', 'success');
    }

    function normalizeToolbar(toolbar) {
        if (!toolbar || toolbar.dataset.favoriteNormalized === '1') return;

        toolbar.classList.add('favorite-pop-toolbar');
        toolbar.dataset.favoriteNormalized = '1';

        TOOLBAR_LABELS.forEach(conf => {
            let btn = toolbar.querySelector('.' + conf.cls);

            // 原工具栏里没有“查看”，这里自动创建
            if (!btn) {
                btn = document.createElement('button');
                btn.className = 'toolbar-btn ' + conf.cls;
                toolbar.appendChild(btn);
            }

            btn.textContent = conf.text;
            btn.style.order = String(conf.order || 10);
        ...
        let favBtn = toolbar.querySelector('.favorite-btn');
        if (!favBtn) {
            favBtn = document.createElement('button');
            favBtn.className = 'toolbar-btn favorite-btn';
            favBtn.textContent = '收藏';
            toolbar.appendChild(favBtn);
        }
        favBtn.style.order = '6';
        ...
        toolbar.addEventListener('click', async e => {
            ...
            if (viewBtn) {
                ...
                const ctx = toolbar.__favCtx || getContextFromTarget(toolbar);
                await openMessageViewModal(ctx);
            ...
            if (favBtn) {
                ...
                const ctx = toolbar.__favCtx || getContextFromTarget(toolbar);
                await toggleFavorite(ctx, favBtn);
                return;
            }
        });
    }
    function closeAllToolbars() {
        document.querySelectorAll('.bubble-toolbar.favorite-pop-toolbar.show').forEach(tb => {
            tb.classList.remove('show');
        });
    }

    async function showToolbarAt(toolbar, x, y, ctx) {
        if (!toolbar) return;
        normalizeToolbar(toolbar);

        toolbar.__favCtx = ctx || null;

        const favBtn = toolbar.querySelector('.favorite-btn');
        if (favBtn && ctx) {
            const fav = await isFavorited(ctx);
    ...
    function isToolbarBubbleTarget(el) {
        return !!(
            el.closest('.group-bubble') ||
            el.closest('.bubble:not(.voice-call-msg)') ||
            el.closest('.offline-card-user') ||
            el.closest('.offline-card-ai') ||
            el.closest('.group-offline-card-user') ||
            el.closest('.group-offline-card-ai') ||
            el.closest('.html-card-frame-wrap') ||
            el.closest('.html-card-bubble') ||
            el.closest('.image-bubble') ||
            el.closest('.voice-bubble') ||
            el.closest('.emoticon-bubble') ||
            el.closest('.gg-transfer-card') ||
            el.closest('.gg-redpacket-card')
        );
    }
    ...
    function bindClickToolbarTrigger() {
        document.addEventListener('click', async function(e) {
            if (e.target.closest('.bubble-toolbar')) return;

            if (!isToolbarBubbleTarget(e.target)) {
                closeAllToolbars();
                return;
            }

            const toolbar = getToolbarFromTarget(e.target);
            const ctx = getContextFromTarget(e.target);

            if (!toolbar || !ctx) return;

            // 阻止原本图片/语音点击展开等事件，统一先弹工具栏
            e.preventDefault();
            e.stopPropagation();
            ...
            const p = getPoint(e);
            await showToolbarAt(toolbar, p.x, p.y, ctx);
        }, true);

        document.addEventListener('scroll', closeAllToolbars, true);
        window.addEventListener('resize', closeAllToolbars);
    }

    /* ========== 收藏页 ========== */

    function ensureFavoritePage() {
        if (document.getElementById('favoritePageModal')) return;

        const modal = document.createElement('div');
        modal.id = 'favoritePageModal';
        modal.className = 'favorite-page-modal';
        modal.innerHTML = `
            <div class="favorite-page-header">
                <button class="favorite-page-back" id="favoritePageBackBtn">←</button>
                <div class="favorite-page-title" id="favoritePageTitle">收藏</div>
                <div class="favorite-page-spacer"></div>
            </div>
            <div class="favorite-search-wrap">
                <input class="favorite-search-input" id="favoriteSearchInput" placeholder="搜索收藏消息或对话">
            </div>
            <div class="favorite-body" id="favoriteBody"></div>
        `;
        document.body.appendChild(modal);

        document.getElementById('favoritePageBackBtn').addEventListener('click', () => {
            const mode = modal.dataset.mode || 'list';
            if (mode === 'detail') {
                renderFavoriteConversationList();
            } else {
                modal.classList.remove('show');
            }
        });

        document.getElementById('favoriteSearchInput').addEventListener('input', () => {
            const mode = modal.dataset.mode || 'list';
            if (mode === 'detail') {
                renderFavoriteMessageList(modal.dataset.sourceType, modal.dataset.sourceId);
            } else {
                renderFavoriteConversationList();
            }
        });
    ...
    async function insertFavoriteEntry() {
        const profileMenu = document.querySelector('#page-profile .menu-list');
        const emoticonBtn = document.getElementById('emoticonEntryBtn');
        if (!profileMenu || !emoticonBtn) return;
        if (document.getElementById('favoriteMessagesEntryBtn')) return;

        const item = document.createElement('div');
        item.className = 'menu-item clickable';
        item.id = 'favoriteMessagesEntryBtn';
        item.innerHTML = `
            <span class="menu-icon">${FAV_ICON}</span>
            <span class="menu-text">收藏</span>
        `;

        profileMenu.insertBefore(item, emoticonBtn);
    ...
    async function openFavoritePage() {
        ensureFavoritePage();
        const modal = document.getElementById('favoritePageModal');
        modal.classList.add('show');
        await renderFavoriteConversationList();
    }

    function getSearchKeyword() {
        return (document.getElementById('favoriteSearchInput')?.value || '').trim().toLowerCase();
    }

    function matchFavorite(item, kw) {
        if (!kw) return true;
        return [
            item.sourceTitle,
            item.senderName,
            item.content,
            item.messageType
        ].some(v => safeText(v).toLowerCase().includes(kw));
    }
    ...
    async function renderFavoriteConversationList() {
        ...
        const modal = document.getElementById('favoritePageModal');
        const body = document.getElementById('favoriteBody');
        const title = document.getElementById('favoritePageTitle');
        ...
        const list = all.filter(item => matchFavorite(item, kw));

        if (!list.length) {
            body.innerHTML = '<div class="favorite-empty">暂无收藏消息</div>';
            return;
        }
        ...
        const groups = Array.from(map.values()).sort((a, b) => {
            const at = a.items[0]?.addedAt || 0;
            const bt = b.items[0]?.addedAt || 0;
            return bt - at;
        });

        let html = '<div class="favorite-conv-list">';
        for (const g of groups) {
            const avatar = await getSourceAvatar({
                sourceType: g.sourceType,
                sourceId: Number(g.sourceId)
            });
            const latest = g.items[0];
            const preview = previewText(latest);
            const imgStyle = avatar.image
                ? `background-image:url('${avatar.image}');background-color:transparent;`
                : `background:${avatar.color};`;

            html += `
                <div class="favorite-conv-item" data-source-type="${g.sourceType}" data-source-id="${g.sourceId}">
                    <div class="favorite-conv-avatar" style="${imgStyle}">${avatar.image ? '' : escapeHtml(avatar.text)}</div>
                    <div class="favorite-conv-info">
                        <div class="favorite-conv-title">${escapeHtml(g.title)}</div>
                        <div class="favorite-conv-preview">${escapeHtml(preview)}</div>
                    </div>
                    <div class="favorite-conv-count">${g.items.length}</div>
                </div>
            `;
        }
        html += '</div>';
        ...
        body.querySelectorAll('.favorite-conv-item').forEach(el => {
            el.addEventListener('click', () => {
                renderFavoriteMessageList(el.dataset.sourceType, el.dataset.sourceId);
            });
        });
    }
    ...
    function previewText(item) {
        ...
        if (type === 'image') return '[图片] ' + safeText(item.content).slice(0, 40);
        if (type === 'voice') return '[语音] ' + safeText(item.content).slice(0, 40);
        if (type === 'html_card') return '[HTML卡片]';
        if (type === 'emoticon') return '[表情包]';
        if (type === 'offline_card' || type === 'offline') return '[线下] ' + safeText(item.content).slice(0, 40);
        return safeText(item.content).replace(/\s+/g, ' ').slice(0, 50);
    }

    async function renderFavoriteMessageList(sourceType, sourceId) {
        ensureFavoritePage();
        ...
        let list = all.filter(item =>
            ...
            matchFavorite(item, kw)
        );

        list.sort((a, b) => (b.addedAt || 0) - (a.addedAt || 0));

        title.textContent = list[0]?.sourceTitle || '收藏消息';

        if (!list.length) {
            body.innerHTML = '<div class="favorite-empty">此对话暂无匹配收藏</div>';
            return;
        }

        let html = '<div class="favorite-msg-list">';
        list.forEach(item => {
            html += renderFavoriteMessageCard(item);
        });
        html += '</div>';

        body.innerHTML = html;

        body.querySelectorAll('.favorite-remove-btn').forEach(btn => {
            btn.addEventListener('click', async e => {
                ...
                await setFavorites(list);
                await renderFavoriteMessageList(sourceType, sourceId);
                window.showStatus && window.showStatus('已取消收藏', 'success');
            });
        });
    }

    function renderFavoriteMessageCard(item) {
        ...
        return `
            <div class="favorite-msg-card" data-fav-id="${escapeHtml(item.id)}">
                <div class="favorite-msg-meta">
                    <span class="favorite-msg-sender">${escapeHtml(item.senderName || '未知')}</span>
                    <span class="favorite-msg-time">${escapeHtml(time)}</span>
                </div>
                ${renderFavoriteContent(item)}
                <div class="favorite-msg-actions">
                    <button class="favorite-remove-btn" data-fav-id="${escapeHtml(item.id)}">取消收藏</button>
                </div>
            </div>
        `;
    }
    ...
    function ensureMessageViewModal() {
        if (document.getElementById('messageViewModal')) return;

        const modal = document.createElement('div');
        modal.id = 'messageViewModal';
        modal.className = 'message-view-modal';
        modal.innerHTML = `
            <div class="message-view-card">
                <div class="message-view-header">
                    <div class="message-view-title" id="messageViewTitle">查看消息</div>
                    <button class="message-view-close" id="messageViewCloseBtn">×</button>
                </div>
                <div class="message-view-body" id="messageViewBody"></div>
            </div>
        `;
        ...
        document.getElementById('messageViewCloseBtn').addEventListener('click', close);
    ...
    async function openMessageViewModal(ctx) {
        if (!ctx || !window.DB) return;
        ...
        const bodyEl = document.getElementById('messageViewBody');

        const sourceTitle = await getSourceTitle(ctx);
        const senderName = msg.senderName || (msg.role === 'user' ? '我' : '对方') || '未知';
        ...
        titleEl.textContent = sourceTitle || '查看消息';
    ...
    function renderViewContent(item) {
        ...
        if (type === 'image') {
            return `
                <div class="message-view-content view-media">
                    <div class="message-view-media-label">图片描述</div>
        ...
        if (type === 'voice') {
            return `
                <div class="message-view-content view-media">
                    <div class="message-view-media-label">语音内容</div>
                    <div>${escapeHtml(content)}</div>
                </div>
            `;
        }
        ...
        if (type === 'html_card') {
            const safe = window.wbE ? window.wbE.sanitize(content) : escapeHtml(content);
            return `
                <div class="message-view-content view-html">
                    ${safe}
                </div>
            `;
        }
        ...
        if (type === 'emoticon') {
            ...
            return `
                <div class="message-view-content view-media">
                    <div class="message-view-media-label">表情包</div>
                    ${url ? `<img src="${escapeHtml(url)}" style="max-width:160px;max-height:160px;border-radius:10px;display:block;margin-bottom:8px;">` : ''}
                    <div>${escapeHtml(text)}</div>
                </div>
            `;
        }

        if (type === 'offline_card' || type === 'offline') {
            return `
                <div class="message-view-content view-offline">
                    ${escapeHtml(content)}
                </div>
            `;
        }
        ...
        if (type === 'diary_share') {
            ...
            if (diaryData) {
                const plain = (diaryData.richContent || diaryData.content || '').replace(/<[^>]*>/g, '');
                return `
                    <div class="message-view-content view-media">
                        <div class="message-view-media-label">日记分享</div>
                        <div style="font-weight:600;margin-bottom:6px;">${escapeHtml(diaryData.title || '无标题')}</div>
                        <div>${escapeHtml(plain)}</div>
                    </div>
                `;
            }
        }
        ...
        return `
            <div class="message-view-content view-text">
                ${escapeHtml(content)}
            </div>
        `;
    }

    function renderFavoriteContent(item) {
        const type = item.messageType || 'text';
        const content = item.content || '';

        if (type === 'image') {
            return `
                <div class="favorite-msg-content fav-media">
                    <div class="favorite-media-label">图片</div>
                    <div>${escapeHtml(content)}</div>
                </div>
            `;
        }
        ...
        if (type === 'voice') {
            return `
                <div class="favorite-msg-content fav-media">
                    <div class="favorite-media-label">语音</div>
                    <div>${escapeHtml(content)}</div>
                </div>
            `;
        }
        ...
        if (type === 'html_card') {
            const safe = window.wbE ? window.wbE.sanitize(content) : escapeHtml(content);
            return `
                <div class="favorite-msg-content fav-html">
                    ${safe}
                </div>
            `;
        }
        ...
        if (type === 'emoticon') {
            ...
            return `
                <div class="favorite-msg-content fav-media">
                    <div class="favorite-media-label">表情包</div>
                    ${url ? `<img src="${escapeHtml(url)}" style="max-width:120px;max-height:120px;border-radius:8px;display:block;margin-bottom:6px;">` : ''}
                    <div>${escapeHtml(text)}</div>
                </div>
            `;
        }
        ...
        if (type === 'offline_card' || type === 'offline') {
            return `<div class="favorite-msg-content fav-offline">${escapeHtml(content)}</div>`;
        }

        if (type === 'transfer' || type === 'redpacket') {
            return `<div class="favorite-msg-content fav-html">${content}</div>`;
        }

        return `<div class="favorite-msg-content fav-text">${escapeHtml(content)}</div>`;
    }
    ...
    function bootstrap() {
        ...
        if (profilePage) {
            ...
            obs.observe(profilePage, { attributes: true, attributeFilter: ['class'] });
        }

        console.log('收藏与点击工具栏增强已启用');
    }
