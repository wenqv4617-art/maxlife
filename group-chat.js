...
(function initGroupChat() {
    ...
    // 群聊 stores 已在主脚本 openDB 的 onupgradeneeded 中创建，无需额外处理
    ...
    async function buildGroupOnlinePrompt(g) {
        ...
        membersInfo.forEach(m => {
            charsSection += `- ${m.name}${m.isOwner?'（群主）':''}${m.isAdmin?'（管理员）':''}${m.title?'（头衔：'+m.title+'）':''}: ${m.detail}\n`;
            if (m.syncMemory && m.recentSummaries) charsSection += `  近期的记忆：${m.recentSummaries.join('；')}\n`;
...
- 直接以叙事文本输出。描述谁做了什么、说了什么、发生了什么。像写小说一样`;
    ...
    async function loadGroupMessages(id) {
            ...
            for (let i = 0; i < ms.length; i++) {
                ...
                if (m.type === 'system') {
                    ...
                    if (m.messageType === 'transfer' || m.messageType === 'redpacket') {
                        ...
                        contentHtml = `<div class="bubble image-bubble" style="background:#fde9ea;text-align:center;">🖼️<br><span style="font-size:12px;color:#d8b69f;">${window.escapeHtml(m.content)}</span></div>`;
                    } else if (m.messageType === 'voice') {
                        contentHtml = `<div class="bubble voice-bubble" style="background:#fde9ea;">🎤 语音<br><span style="font-size:12px;color:#d8b69f;">${window.escapeHtml(m.content)}</span></div>`;
                    } else {
                        const quoteMatch = m.content.match(/^「(.+?)」\n/);
                        if (quoteMatch) {
                            const mainText = m.content.replace(/^「.+?」\n/, '');
                            contentHtml = `<div class="group-bubble quoted-bubble"><div class="quote-ref-header">↩️ ${window.escapeHtml(quoteMatch[1])}</div>${window.escapeHtml(mainText)}</div>`;
                        } else {
                            contentHtml = `<div class="group-bubble">${window.escapeHtml(m.content)}</div>`;
                        }
                    }
                    ...
                    if (m.messageType === 'emoticon') {
                        ...
                        bubbleContent = `<div class="bubble image-bubble">🖼️<br><span style="font-size:12px;color:#d8b69f;">${window.escapeHtml(m.content)}</span></div>`;
                    } else if (m.messageType === 'voice') {
                        bubbleContent = `<div class="bubble voice-bubble">🎤 语音<br><span style="font-size:12px;color:#d8b69f;">${window.escapeHtml(m.content)}</span></div>`;
                    } else {
                        bubbleContent = `<div class="group-bubble">${window.escapeHtml(m.content)}</div>`;
                    }
        ...
        setTimeout(() => { ct.scrollTop = ct.scrollHeight; bindGroupCardClicks(ct); }, 100);
    }
    ...
    function quoteMessage(idx, msgs) {
        ...
        const previewBar = document.getElementById('groupQuotePreviewBar');
        const previewContent = document.getElementById('groupQuotePreviewContent');
        if (previewBar && previewContent) {
            previewContent.textContent = `${quotedMessage.senderName}: ${previewText}`;
    ...
    function bindGroupBubbleToolbar(container, msgs) {
        container.querySelectorAll('.bubble-dot').forEach(dot => {
            dot.addEventListener('click', (e) => {
                e.stopPropagation();
                const parent = dot.closest('.group-message-row') || dot.closest('[data-offline-msg-id]');
                const toolbar = parent.querySelector('.bubble-toolbar');
        ...
        container.querySelectorAll('.reback-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                if (!msg) return;
                let lastUserIdx = -1;
                for (let i = idx - 1; i >= 0; i--) { if (msgs[i]?.senderId === 'user') { lastUserIdx = i; break; } }
                if (lastUserIdx === -1) { window.showStatus('无法找到对应的用户消息', 'error'); return; }
                if (!confirm('确定要回到这条消息并删除之后的所有回复吗？')) return;
                for (let i = lastUserIdx + 1; i < msgs.length; i++) { if (msgs[i]?.id) await window.DB.delete('groupMessages', msgs[i].id); }
                btn.closest('.bubble-toolbar').classList.remove('show');
                await loadGroupMessages(window.currentGroupId);
        ...
        container.querySelectorAll('.edit-msg-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                const idx = parseInt(btn.dataset.idx);
                const msg = msgs[idx];
                if (!msg || (msg.messageType && msg.messageType !== 'text')) {
                    window.showStatus('只能编辑文字消息', 'info'); return;
                }
        ...
        container.querySelectorAll('.group-message-row').forEach(row => {
            let tx = 0, ty = 0, cr = null;
            row.addEventListener('touchstart', (e) => { tx = e.touches[0].clientX; ty = e.touches[0].clientY; cr = row; }, { passive: true });
    ...
    async function openGroupConversation(id) {
        ...
        if (window.switchPage) window.switchPage('group-conversation');
    }
            ...
            if (mode === 'offline') {
                ...
                for (const line of lines) {
                    ...
                    if (match) {
                        ...
                        const msgRegex = /\[MSG\](文字|图片|语音|表情包):\s*([^\[\]]+?)(?=\[MSG\]|$)/g;
                        ...
                        while ((execResult = msgRegex.exec(remaining)) !== null) {
                            ...
                            const typeMap = { '文字': 'text', '图片': 'image', '语音': 'voice', '表情包': 'emoticon' };
    ...
    function showRedPacketModal() {
        const rpTypeMulti = document.getElementById('rpTypeMulti');
        const rpTypeSingle = document.getElementById('rpTypeSingle');
        if (rpTypeMulti) rpTypeMulti.classList.add('selected');
        if (rpTypeSingle) rpTypeSingle.classList.remove('selected');
        const rpMultiSettings = document.getElementById('rpMultiSettings');
        const rpSingleSettings = document.getElementById('rpSingleSettings');
        if (rpMultiSettings) rpMultiSettings.style.display = '';
        if (rpSingleSettings) rpSingleSettings.style.display = 'none';
        rpMode = 'multi';
        const rpModeNormal = document.getElementById('rpModeNormal');
        const rpModeLucky = document.getElementById('rpModeLucky');
        if (rpModeNormal) rpModeNormal.classList.add('selected');
        if (rpModeLucky) rpModeLucky.classList.remove('selected');
        const rpMessage = document.getElementById('rpMessage');
        if (rpMessage) {
            rpMessage.placeholder = '恭喜发财！';
            rpMessage.value = '';
        }
    ...
    async function openGroupDetail() {
        ...
        if (window.switchPage) window.switchPage('group-detail');
    }
    ...
    async function showGroupWorldbookModal() {
        ...
        container.innerHTML = allWorldbooks.length === 0 ? '<p style="color:#a0a8a2;padding:12px;">暂无世界书</p>' : allWorldbooks.map(wb => `<label class="mount-checkbox"><input type="checkbox" value="${wb.id}" class="group-wb-checkbox" ${selectedIds.includes(wb.id)?'checked':''}><div><div style="font-weight:500;">${window.escapeHtml(wb.title)}</div><div style="font-size:11px;color:#8ba3c7;">${window.escapeHtml((wb.content||'').substring(0,50))}...</div></div></label>`).join('');
    ...
    async function saveGroupWorldbook() {
        ...
        if (!g) return;
        const ids = [];
        document.querySelectorAll('.group-wb-checkbox:checked').forEach(cb => ids.push(cb.value));
        g.worldbookIds = ids;
        await window.DB.put('groupChats', g);
    ...
    async function renderGroupEmoticonPicker() {
        ...
        if (!tc) return;
        tc.innerHTML = gs.length ? gs.map((grp, i) => `<button class="emoticon-picker-tab ${i===0?'active':''}" data-gid="${grp.id}">${window.escapeHtml(grp.name)}</button>`).join('') : '<span style="font-size:12px;color:#a0a8a2;padding:8px;">未挂载表情包</span>';
    ...
    async function confirmMultiSelect() {
        ...
        const setupYes = document.getElementById('setupParticipateYes');
        const setupNo = document.getElementById('setupParticipateNo');
        if (setupYes && !setupYes.classList.contains('selected') && !setupNo.classList.contains('selected')) {
    setupYes.classList.add('selected');
}
        if (setupNo) setupNo.classList.remove('selected');
        const os = document.getElementById('groupSetupOwner');
const isParticipate = setupYes.classList.contains('selected');
if (isParticipate) {
    ...
    async function confirmGroupSetup() {
        ...
        if (typeof window.refreshConversationList === 'function') await window.refreshConversationList();
    ...
    function bindEvents() {
        ...
        document.getElementById('backFromGroupDetailBtn')?.addEventListener('click', () => {
            if (window.switchPage) window.switchPage('group-conversation');
        });

        // 红包弹窗
        document.getElementById('cancelRedPacketBtn')?.addEventListener('click', () => {
            document.getElementById('groupRedPacketModal').classList.remove('active');
        });
        document.getElementById('confirmRedPacketBtn')?.addEventListener('click', confirmRedPacket);

        // 转账弹窗
        document.getElementById('cancelTransferTargetBtn')?.addEventListener('click', () => {
            document.getElementById('groupTransferTargetModal').classList.remove('active');
        });

        // 红包类型切换
        document.getElementById('rpTypeMulti')?.addEventListener('click', () => {
            rpMode = 'multi';
        ...
        document.getElementById('rpTypeSingle')?.addEventListener('click', async () => {
            rpMode = 'single';
            ...
            document.getElementById('rpSingleSettings').style.display = '';
            const g = await window.DB.get('groupChats', window.currentGroupId);
            if (g) {
                const s = document.getElementById('rpSingleTarget');
                s.innerHTML = '';
                for (const mid of g.memberIds) {
                    const ch = await window.DB.get('characters', mid);
                    if (ch) {
        ...
        // 红包模式切换
        document.getElementById('rpModeNormal')?.addEventListener('click', () => {
            document.getElementById('rpModeNormal').classList.add('selected');
            document.getElementById('rpModeLucky').classList.remove('selected');
        });
        document.getElementById('rpModeLucky')?.addEventListener('click', () => {
            document.getElementById('rpModeLucky').classList.add('selected');
        ...
        document.getElementById('groupWorldbookCancelBtn')?.addEventListener('click', () => {
            document.getElementById('groupWorldbookModal').classList.remove('active');
        });
        ...
        document.getElementById('groupEmoticonMountCancelBtn')?.addEventListener('click', () => {
            document.getElementById('groupEmoticonMountModal').classList.remove('active');
        });
        document.getElementById('groupEmoticonMountSaveBtn')?.addEventListener('click', saveGroupEmoticonMount);

        // 禁言弹窗
        document.getElementById('muteMemberCancelBtn')?.addEventListener('click', () => {
            document.getElementById('muteMemberModal').classList.remove('active');
        });
        document.getElementById('muteMemberConfirmBtn')?.addEventListener('click', confirmMuteMember);

        // 群聊总结弹窗
        document.getElementById('groupSummaryCloseBtn')?.addEventListener('click', () => {
            document.getElementById('groupSummaryModal').classList.remove('active');
        ...
        // 返回群聊列表
        document.getElementById('backFromGroupConvBtn')?.addEventListener('click', () => {
        ...
        document.getElementById('saveGroupNPCBtn')?.addEventListener('click', async () => {
            ...
            const detail = document.getElementById('groupNPCDetail').value.trim();
            if (!name) return;
            ...
            if (nid) {
                ...
                if (npc) {
                    ...
                    npc.avatar = avatar;
                    npc.detail = detail;
                    await window.DB.put('groupNPCs', npc);
                }
            } else {
                const newNPC = { groupId: window.currentGroupId, name, avatar, detail, createdAt: Date.now() };
                const npcId = await new Promise((resolve, reject) => {
                    const req = indexedDB.open('CompanionDB_V18', 20);
                    req.onsuccess = e => {
                        const db = e.target.result;
                        const tx = db.transaction('groupNPCs', 'readwrite');
                        const addReq = tx.objectStore('groupNPCs').add(newNPC);
                ...
                const g = await window.DB.get('groupChats', window.currentGroupId);
                if (g) {
                    if (!g.memberIds.includes(String(npcId))) {
                        g.memberIds.push(String(npcId));
                        if (!g.members) g.members = [];
                        g.members.push({ id: String(npcId), title: '', syncMemory: false, avatar });
                        await window.DB.put('groupChats', g);
                    }
                }
            }
            document.getElementById('groupNPCEditModal').classList.remove('active');
            if (window.openGroupDetail) window.openGroupDetail();
        });

        document.getElementById('cancelGroupNPCBtn')?.addEventListener('click', () => {
            document.getElementById('groupNPCEditModal').classList.remove('active');
        });

        document.getElementById('deleteGroupNPCBtn')?.addEventListener('click', async () => {
            if (!confirm('确定删除该NPC吗？')) return;
            const npcId = parseInt(document.getElementById('groupNPCEditId').value);
            await window.DB.delete('groupNPCs', npcId);
            document.getElementById('groupNPCEditModal').classList.remove('active');
            if (window.openGroupDetail) window.openGroupDetail();
        });

        // ========== 群聊头像上传 ==========
        document.getElementById('groupAvatarUploadBtn')?.addEventListener('click', () => {
        ...
        // ========== 群聊背景上传 ==========
        document.getElementById('groupBgUploadBtn')?.addEventListener('click', () => {
            document.getElementById('groupBgFile').click();
        });
        ...
        // ========== NPC 头像上传 ==========
        document.getElementById('npcAvatarUploadBtn')?.addEventListener('click', () => {
            document.getElementById('npcAvatarFile').click();
        });
        ...
        // ========== 扩容菜单 ==========
        document.getElementById('groupExpandMenuBtn')?.addEventListener('click', async () => {
            const menu = document.getElementById('groupExpandMenu');
            const g = window.currentGroupId ? await window.DB.get('groupChats', window.currentGroupId) : null;
            if (!g) { menu.classList.toggle('active'); return; }
            if (g.isUserParticipating) {
                menu.innerHTML = `
                    <div class="expand-menu-item" data-action="groupVoice"><span class="expand-menu-icon">🎤</span><span class="expand-menu-label">语音</span></div>
                    <div class="expand-menu-item" data-action="groupImage"><span class="expand-menu-icon">🖼️</span><span class="expand-menu-label">图片</span></div>
                    <div class="expand-menu-item" data-action="groupEmoticon"><span class="expand-menu-icon">😊</span><span class="expand-menu-label">表情</span></div>
                    <div class="expand-menu-item" data-action="groupToggleMode"><span class="expand-menu-icon">🔄</span><span class="expand-menu-label">见面</span></div>
                    <div class="expand-menu-item" data-action="groupTransfer"><span class="expand-menu-icon">💸</span><span class="expand-menu-label">转账</span></div>
                    <div class="expand-menu-item" data-action="groupRedPacket"><span class="expand-menu-icon">🧧</span><span class="expand-menu-label">发红包</span></div>
                    <div class="expand-menu-item" data-action="groupSummary"><span class="expand-menu-icon">📋</span><span class="expand-menu-label">总结</span></div>
                    <div class="expand-menu-item" data-action="groupOpenDetail"><span class="expand-menu-icon">📝</span><span class="expand-menu-label">详情</span></div>
                `;
            } else {
                menu.innerHTML = `
                    <div class="expand-menu-item" data-action="groupSummary"><span class="expand-menu-icon">📋</span><span class="expand-menu-label">总结</span></div>
        ...
        function bindExpandMenuActions() {
            document.querySelectorAll('#groupExpandMenu .expand-menu-item').forEach(el => {
                el.addEventListener('click', () => {
                    ...
                    if (a === 'groupVoice') {
                        const c = prompt('语音内容：');
                        if (c) {
                            const maskPromise = window.getActiveMask ? window.getActiveMask() : Promise.resolve({ name: '我' });
                            maskPromise.then(mask => {
                                window.DB.put('groupMessages', {
                                    groupId: window.currentGroupId,
                                    senderId: 'user', senderName: mask.name || '我', role: 'user',
                                    content: c, messageType: 'voice', timestamp: Date.now()
                                });
                        ...
                        if (d) {
                            ...
                            maskPromise.then(mask => {
                                window.DB.put('groupMessages', {
                                    groupId: window.currentGroupId,
                                    senderId: 'user', senderName: mask.name || '我', role: 'user',
                                    content: d, messageType: 'image', timestamp: Date.now()
                                });
                                loadGroupMessages(window.currentGroupId);
                            });
                        }
    ...
    window.confirmMultiSelect = confirmMultiSelect;
window.confirmGroupSetup = confirmGroupSetup;
