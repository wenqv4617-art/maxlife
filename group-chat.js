...
function renderQuotedGroupBubble(rawContent) {
    ...
    const mainText = content.replace(/^「.+?」\n/, '');

    return `
        <div class="group-bubble quoted-bubble">
            <div>${window.escapeHtml(mainText)}</div>
            <div class="quote-ref-footer">
                <div class="quote-ref-footer-title">
                    <span style="display:inlineHtml(quoteInfo)}</div>
            </div>
        </div>
    `;
}
...
function renderGroupMediaBubble(kind, content) {
    ...
    const icon = isImage ? svgIcon('image') : svgIcon('mic');
    const title = isImage ? '图片' : '语音消息';
    const desc = isImage ? (content || '图片描述') : (content || '点击查看语音内容');

    return `
        <div class="bubble ${isImage ? 'image-bubble' : 'voice-bubble'} media-bubble">
            <div class="media-bubble-row">
                <span class="media-bubble-icon">${icon}</span>
                <div class="media-bubble-main">
                    <div class="media-bubble-title">${title}</div>
                    <div class="media-bubble-desc">${window.escapeHtml(desc)}</div>
                </div>
            </div>
        </div>
    `;
