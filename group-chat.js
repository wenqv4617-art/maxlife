...
await loadGroupMessages(id);
    ...
    async function showGroupWorldbookModal() {
        ...
        const hint = `<div class="wb-priority-hint">
    ...
    allWorldbooks.length === 0
        ...
        : allWorldbooks.map(wb => `
            <label class="mount-checkbox wb-mount-checkbox" style="align-items:flex-start;">
                <input type="checkbox" value="${wb.id}" class="group-wb-checkbox" ${selectedIds.includes(wb.id)?'checked':''}>
                <div style="flex:1;min-width:0;">
                    <div class="wb-mount-title">${window.escapeHtml(wb.title)}</div>
                    <div class="wb-mount-preview">${window.escapeHtml((wb.content||'').substring(0,50))}${(wb.content||'').length > 50 ? '...' : ''}</div>
                </div>
            </label>
        `).join('')
);
