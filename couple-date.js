...
(function () {
  ...
  async function onGenerateWorld() {
    ...
    try {
      ...
      data.lastCfg = { userRole: cfg.userRole, charRole: cfg.charRole, wordCount: cfg.wordCount, storyWordCount: cfg.storyWordCount };
      await saveData(state.convId, data);
...
【核心人物锚点 - 这两个人将以新身份在小世界登场，但保留人物内核（性格、关键设定、说话风格）】
...
2. 完整原文剧情：约 ${cfg.storyWordCount} 字。
   ...
   - 必须有起承转合，包含完整结局
   - 网文白妙文风：叙述自然，句子长短自由，不刻意分段，禁止 *动作* 或 (动作) 形式

3. 在该小世界里 ${ctx.charName} 的具体身份：包括化名、社会身份、与小世界主线的关系。要保留 ${ctx.charName} 的性格内核与关键锚点（比如原本是现代霸道总裁，到了古代可以是摄政王或富商，但说一不二的性格不变）。
  ...
  async function renderWorld() {
    ...
    scroll.innerHTML = `
      ...
      <div class="dt-input-row">
        <input type="text" id="dtCustomInput" class="dt-input" placeholder="或自由输入行动…">
