...
(function() {
    ...
    function closePhoneDetail() { document.getElementById('phoneDetailPage').classList.remove('active'); document.getElementById('phoneFullscreenApp').classList.add('active'); document.getElementById('phoneDetailClearBtn').style.display='none'; window._currentDiaryIdx=undefined; }
...
const reply = await callLLM([{role:'user',content:prompt}], {maxTokens:2500,temperature:0.9});
    ...
    async function buildPhoneRegeneratePrompt(appType, extra={}) {
        ...
        const prompts = {
            message: `${base}\n${(()=>{const chats=extra.existingChats||[]; if(!chats.length)return'（目前没有对话）'; return chats.map((c,i)=>`${i+1}. "${c.contactName}"：\n`+(c.messages||[]).slice(-6).map(m=>`  ${m.role==='self'?'角色':c.contactName}: ${m.text}`).join('\n')).join('\n\n');})()}\n\n延续已有对话或创建新对话。不要生成置顶联系人内容。输出JSON: {"pinnedNickname":"可选","messages":[...]}`,
            browser: `${base}\n\n生成5-8条搜索历史。输出JSON: {"history":[{"text":"","time":""}]}`,
            browser_search: `${base}\n\n搜索"${extra.searchText||''}"，生成4-5篇300-500字长文帖子带评论。JSON: {"posts":[...]}`,
            memo: `${base}\n\n生成5-8条新备忘录，全部未完成。JSON: {"memos":[{"text":"","checked":false}]}`,
            diary: `${base}\n\n以角色口吻写一篇300-500字日记。JSON: {"diaries":[{"title":"","date":"","content":""}]}`,
            forum: `${base}\n\n生成3-5篇论坛帖子带多样化评论。JSON: {"posts":[...]}`,
            shop: `${base}\n\n购物车2-4件，资产5-8条。JSON: {"cart":[],"assets":[],"totalAssets":""}`,
            stats: `${base}\n\n屏幕使用统计，5-7个App，时间格式"Xh Xmin"。JSON: {"totalTime":"","apps":[]}`,
        };
    ...
    async function applyPhoneRegenerateData(appType, parsed) {
        ...
        switch (appType) {
            case 'message':
                if (parsed.pinnedNickname) { const p=data.messages.find(c=>c.isPinned); if(p)p.contactName=parsed.pinnedNickname; }
                if (parsed.messages) { if(!data.messages)data.messages=[]; parsed.messages.forEach(c=>{if(c.isPinned)return;const k=(c.contactName||'').replace(/\s+/g,'').toLowerCase();const e=data.messages.find(m=>!m.isPinned&&(m.contactName||'').replace(/\s+/g,'').toLowerCase()===k);if(e){e.messages=[...(e.messages||[]),...(c.messages||[])];e.time=c.time||e.time;}else{data.messages.push({contactName:c.contactName,messages:c.messages||[],time:c.time||'',isPinned:false});}}); }
                break;
            ...
            case 'shop': if(parsed.cart)data.shopCart=[...parsed.cart,...(data.shopCart||[])]; if(parsed.assets)data.shopAssets=[...parsed.assets,...(data.shopAssets||[])]; if(parsed.totalAssets)data.shopTotalAssets=parsed.totalAssets; break;
    ...
    async function syncUserChatToPhone(data) {
        ...
        const synced=dc.map(c=>({role:c.role==='user'?'other':'self',text:c.content||''}));
        let uc=data.messages.find(c=>c.isPinned);
        if(!uc){uc={contactName:un,messages:[],time:'',isPinned:true};data.messages.unshift(uc);}
        uc.messages=synced; if(synced.length)uc.time='现在';
        await savePhoneData(data);
    }
    ...
    async function renderPhoneMessages() {
        ...
        all.forEach((c,i)=>{
            ...
            html+=`<div class="phone-chat-item" data-chat-idx="${i}" style="${c.isPinned?'background:#f0f0f0;':''}"><div class="phone-chat-avatar" style="background-color:${getAvatarColor(c.contactName)}">${c.contactName.charAt(0)}</div><div class="phone-chat-info"><div class="phone-chat-name-row"><div class="phone-chat-name">${escapeHtml(c.contactName)}</div><div class="phone-chat-time">${escapeHtml(c.time||'')}</div></div><div class="phone-chat-preview">${escapeHtml(prev)}</div></div></div>`;
        });
    ...
    function openPhoneChatDetail(idx) {
        getPhoneData(currentPhoneCharId).then(data=>{
            const chat=data.messages[idx]; if(!chat)return;
            document.getElementById('phoneDetailTitle').textContent=chat.contactName;
            const h='<div class="phone-chat-messages">'+(chat.messages||[]).map(m=>`<div class="phone-message-item ${m.role==='self'?'right':'left'}"><div class="phone-message-avatar" style="background-color:${m.role==='self'?'#72c9bf':'#ffc107'}">${m.role==='self'?'我':chat.contactName.charAt(0)}</div><div class="phone-message-bubble">${escapeHtml(m.text)}</div></div>`).join('')+'</div>';
