...
;(function () {
    ...
    async function getDB() {
        let d = window._stickerDB;
        if (d && d.objectStoreNames.contains(STORE_GROUPS) && d.objectStoreNames.contains(STORE_STICKERS)) {
            return d;
        }
        // 先尝试打开现有连接
        try {
        ...
        const needUpgrade = !d || !d.objectStoreNames.contains(STORE_GROUPS) || !d.objectStoreNames.contains(STORE_STICKERS);
        if (needUpgrade) {
            if (d) d.close();
            const currentVersion = d ? d.version : 20;
            d = await new Promise((resolve, reject) => {
                const req = indexedDB.open("CompanionDB_V18", currentVersion + 1);
                req.onupgradeneeded = (e) => {
                    const db = e.target.result;
                    if (!db.objectStoreNames.contains(STORE_GROUPS)) {
                        const gs = db.createObjectStore(STORE_GROUPS, { keyPath: 'id' });
                    }
