...
(function () {
  ...
  function renderMineTab(data) {
    ...
    return `
      <div class="cf-console">
        ...
        <input type="number" id="cfWordCount" class="cf-input" placeholder="例如 1500" value="1500" min="200" max="6000">
  ...
  function bindHomeEvents() {
    ...
    if (cmBtn) cmBtn.onclick = onCommissionMyClick;
  ...
  async function onCommissionMyClick() {
    ...
    const wordCount = parseInt(wcEl?.value) || 1500;
    if (wordCount < 200 || wordCount > 6000) { toast("字数请在 200~6000 之间", "error"); return; }
...
【字数要求】
约 ${wordCount} 字。必须有起承转合：开端铺设场景与人物动机，发展推进矛盾或情感张力，高潮处情绪饱满细节充足，结局完整收束。

【写作风格】
- 网文白妙文风。语气放松，叙述自然，不要端着。
