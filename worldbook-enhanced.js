...
(function() {
    ...
    function resolve(opts) {
        var result = { before: "", middle: "", after: "", hasHtml: false };
        ...
        for (var m = 0; m < mounted.length; m++) {
            ...
            if (w.group === "HTML") {
                ...
                text +=
    "\n\n[HTML 卡片输出规则]\n" +
    "触发该主题时，请输出：[MSG]html_card:<完整 HTML>\n" +
    "- 整段 [MSG]html_card: 后面到下一个 [MSG] 之前的内容都会被识别，支持多行\n" +
    "- 允许 <style>，可使用 @keyframes / :hover / transition 做动画\n" +
    "- 允许 <details>/<summary> 实现可点击展开折叠\n" +
    "- 允许 <a href=\"http(s)://...\">\n" +
    "- 自定义 CSS 时，所有选择器都必须套在你自己定义的根类下(例如 .my-card xxx)，禁止裸写标签或全局类污染界面\n" +
    "- 禁止：<script>、<iframe>、on* 事件属性、javascript: 协议、Markdown 代码块包裹(```)\n" +
    "- 推荐工具类：card-header / card-body / card-footer / card-tag / card-divider / card-highlight / card-grid / card-grid-item / card-progress / card-progress-bar";
            }
